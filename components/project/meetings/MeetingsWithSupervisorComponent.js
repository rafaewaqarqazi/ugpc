...
const MeetingsWithSupervisorComponent = ({meetings, role}) => {
  ...
  return (
    <div>
              ...
              !meetings || meetings.length === 0 ?
                ...
                meetings.map((meeting, index) => (
                  <TableRow key={index} className={tableClasses.tableRow}>
                    ...
                    <TableCell>
                      <Chip label={meeting.isAttended ? 'Attended' : 'Not Attended'}
      ...
      <Dialog open={dialog.scheduleMeeting} onClose={() => setDialog({...dialog, scheduleMeeting: false})} fullWidth
              maxWidth='xs' classes={{paper: dialogClasses.root}}>
        {loading && <LinearProgress/>}
        <DialogTitleComponent title={'Schedule Meeting'}
      ...
      <Dialog open={dialog.requestMeeting} onClose={() => setDialog({...dialog, requestMeeting: false})} fullWidth
              maxWidth='xs' classes={{paper: dialogClasses.root}}>
        {loading && <LinearProgress/>}
        <DialogTitleComponent title={'Request for Meeting'}
                              handleClose={() => setDialog({...dialog, requestMeeting: false})}/>
        <DialogContent dividers>
          <TextField
            label='Purpose'
            required
