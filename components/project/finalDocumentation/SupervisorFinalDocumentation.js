...
const SupervisorFinalDocumentation = () => {
  ...
  const ActionsMenu = () => (
    <div>
      ...
      }
      {
        ...
        <MenuItem
          onClick={() => handleClickChangeStatusMenu('Available for Internal', currentDoc.projectId, currentDoc.docId)}>
          <ListItemIcon>
            <SendOutlined/>
          </ListItemIcon>
          <Typography variant="inherit" noWrap>
  ...
  return (
    <div>
                          ...
                          doc.documentation.finalDocumentation.map(finalDoc => (
                            <TableRow key={index} className={classes.tableRow}>
                              ...
                              <TableCell align="center">
                                <Tooltip title='Click to View/Download Document' placement="top"
                                         TransitionComponent={Zoom}>
                                  <a style={{textDecoration: 'none', color: 'grey'}}
                                     href={`${serverUrl}/../pdf/${finalDoc.document.filename}`} target="_blank">
                                    <PictureAsPdfOutlined/>
                                  </a>
                                </Tooltip>
                              </TableCell>
                              ...
                              <TableCell align="center">
                                {
                                  finalDoc.plagiarismReport ?
                                    <Tooltip title='Click to View/Download Document' placement="top"
                                             TransitionComponent={Zoom}>
                                      <a style={{textDecoration: 'none', color: 'grey'}}
                                         href={`${serverUrl}/../pdf/${finalDoc.plagiarismReport.filename}`}
                                         target="_blank">
                                        <PictureAsPdfOutlined/>
                                      </a>
                                    </Tooltip>
                              ...
                              <TableCell>
                                <Tooltip title='Click for Actions' placement='top'>
                                  <IconButton size='small'
                                              onClick={(event) => openMenu(finalDoc.status, finalDoc._id, doc._id, event)}>
                                    <MoreVertOutlined/>
                                  </IconButton>
                                </Tooltip>
                                {
      ...
      <Dialog fullWidth maxWidth='xs' open={uploadDialog.open}
              onClose={() => setUploadDialog({...uploadDialog, open: false})} classes={{paper: dialogClasses.root}}>
        {loading.dialog && <LinearProgress/>}
        <SuccessSnackBar open={success} message={'Success'} handleClose={() => setSuccess(false)}/>
        <DialogTitleComponent title={'Upload Plagiarism Report'}
                              handleClose={() => setUploadDialog({...uploadDialog, open: false})}/>
        <DialogContent>
          <DropzoneArea
