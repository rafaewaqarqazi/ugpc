    ...
    <div>
      ...
      <div className={classes.actions}>
        ...
        </FormControl>
        {
          ...
          <Typography variant='subtitle1'
                      color='textSecondary'>{`${moment(dates.startDate).format('MMM DD, YYYY')} - ${moment(dates.endDate).format('MMM DD, YYYY')}`}</Typography>
        }
        {
          sprintNames.length !== 0 &&
                ...
                <Grid item xs={6} sm={3} key={column.id}>
                  <div className={classes.columnHeader}>
                    ...
                    <Typography variant='caption' color='textSecondary'
                                className={classes.count}>{column.tasksIds.length}</Typography>
                  </div>
      ...
      <Dialog fullWidth maxWidth='sm' open={openDetails} onClose={closeDetails} classes={{paper: dialogClasses.root}}>
        ...
        <DialogContent dividers>
          {
            openDetails &&
            <RenderTaskDetails setDetails={setDetails} details={details} taskIn='ScrumBoard'
                               sprintId={sprint.filter(f => f.name === selectedSprint)[0]._id}/>
          }
