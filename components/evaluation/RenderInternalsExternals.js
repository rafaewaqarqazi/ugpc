...
const RenderInternalsExternals = ({projects, marks, type, fetchData}) => {
  ...
  const handleConfirmEvaluation = () => {
    if (data.marks === '' || data.marks < 0 || data.marks > (type === 'internal' ? marks.internal : marks.external)) {
      setError({
        show: true,
        message: `Provide Marks in 0-${type === 'internal' ? marks.internal : marks.external} Limit`
      })
    } else {
      setLoading({
        ...loading,
                      ...
                      <TableCell align="center">
                        <Tooltip title='Click to View/Download Document' placement="top" TransitionComponent={Zoom}>
                          <a style={{textDecoration: 'none', color: 'grey'}}
                             href={`${serverUrl}/../pdf/${project.documentation.finalDocumentation.document.filename}`}
                             target="_blank">
                            <PictureAsPdfOutlined/>
                          </a>
                        </Tooltip>
                      ...
                      <TableCell align="center">
                        {
                          project.documentation.finalDocumentation.plagiarismReport ?
                            <Tooltip title='Click to View/Download Document' placement="top" TransitionComponent={Zoom}>
                              <a style={{textDecoration: 'none', color: 'grey'}}
                                 href={`${serverUrl}/../pdf/${project.documentation.finalDocumentation.plagiarismReport.filename}`}
                                 target="_blank">
                                <PictureAsPdfOutlined/>
                              </a>
                            </Tooltip>
                      ...
                      <TableCell align="left">
                        <Tooltip title='Click for Actions' placement='top'>
                          <IconButton size='small'
                                      onClick={(event) => handleClickActionMenu(project.documentation.finalDocumentation.status, project.documentation.finalDocumentation._id, project._id, event)}>
                            <MoreVertOutlined/>
                          </IconButton>
                        </Tooltip>
                        <Menu
