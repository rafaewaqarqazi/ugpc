...
const RenderCommitteeDepartmentsComponent = ({
                                               members = {
                                                 id: '',
                                                 members: []
                                               }, department, committeeType, setSuccess
                                             }) => {
  ...
  return (
    <div>
              ...
              filter.length === 0 ?
                ...
                filter.map((member, index) => (
                  ...
                  <TableRow key={index} className={tableClasses.tableRow}>
                    ...
                    <TableCell
                      align="left">{member.ugpc_details.designation ? member.ugpc_details.designation : 'Not Provided'}</TableCell>
                    <TableCell align="left">
                      <Tooltip title='more' placement='top'>
                        <IconButton size='small'
                                    onClick={(event) => handleClickActionMenu(member._id, member.ugpc_details.committees, event)}>
                          <MoreVertOutlined/>
                        </IconButton>
                      </Tooltip>
                      <Menu
      ...
      <Dialog open={dialog.addMember} onClose={() => setDialog({...dialog, addMember: false})} fullWidth maxWidth='sm'
        ...
        <ErrorSnackBar open={error.addMember.show} message={error.addMember.message}
                       handleSnackBar={() => setError({...error, addMember: {show: false, message: ''}})}/>
      ...
      <Dialog open={dialog.removeMember} onClose={() => setDialog({...dialog, removeMember: false})} fullWidth
              maxWidth='xs' classes={{paper: dialogClasses.root}}>
        {loading.remove && <LinearProgress/>}
        <ErrorSnackBar open={error.remove.show} message={error.remove.message}
                       handleSnackBar={() => setError({...error, remove: {show: false, message: ''}})}/>
