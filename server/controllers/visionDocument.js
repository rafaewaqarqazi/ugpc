  ...
  try {
    ...
    const results = await Projects.aggregate([
      {$match: {department: {$in: dep}}},
      {
        $project: {
          students: 1,
          "documentation.visionDocument": 1,
          title: 1,
    ...
    const result = await Projects.populate(results, [
      ...
      {
        path: "projects.documentation.visionDocument.comments.author",
        model: 'Users',
        select: '_id name role department profileImage'
      },
...
exports.fetchBySupervisor = async (req, res) => {
  try {
    ...
    const result = await Projects.populate(results, [
      ...
      {
        path: "documentation.visionDocument.comments.author",
        model: 'Users',
        select: '_id name role department profileImage'
      },
...
exports.scheduleVisionDefence = async (req, res) => {
  try {
    ...
    let emails = [];
    await students.map(student => student.students.map(e => {
